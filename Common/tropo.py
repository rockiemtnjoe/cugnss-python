...
def tropo(sinel, hsta, p, tkel, hum, hp, htkel, hhum):
    ...
    while True:
        ...
        alpha[1] = 2 * a**2 + 4 * b / 3
        alpha[2] = a * (a**2 + 3 * b)
        alpha[3] = a**4 / 5 + 2.4 * a**2 * b + 1.2 * b**2
        alpha[4] = 2 * a * b * (a**2 + 3 * b) / 3
        alpha[5] = b**2 * (6 * a**2 + 4 * b) * 1.428571e-1
        ...
        # dr = rtop; dr = dr + alpha*rn;
        dr = rtop + np.dot(alpha, rn)
